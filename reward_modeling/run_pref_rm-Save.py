...
#         # 生成包含epoch和acc信息的模型名称
#         model_name = f"model_epoch_{state.epoch}_acc_{current_acc:.3f}"
...
#         logger.info(f"Model saved to {model_path}, accuracy: {current_acc:.3f}")

# 按step进行评估和保存
class SaveModelCallback(TrainerCallback):
    """A custom callback that saves the model at specified training steps and evaluates model performance."""

    def __init__(self, tokenizer, save_steps=30):
        self.tokenizer = tokenizer  # 保存分词器的引用
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step % self.save_steps == 0:
            ...
            current_acc = eval_results.get('eval_accuracy', 0)
            # 使用指标信息生成模型名称
            model_name = f"model_step_{state.global_step}_acc_{current_acc:.3f}"
            model_path = os.path.join(args.output_dir, model_name)
            # 保存模型和分词器
            kwargs['model'].save_pretrained(model_path)
            self.tokenizer.save_pretrained(model_path)
            logger.info(f"Model saved to {model_path} at step {state.global_step} with Accuracy: {current_acc:.3f}")
